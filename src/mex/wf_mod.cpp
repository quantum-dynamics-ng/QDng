...
/**
    ...
    * input: 1)three or more input-arguments -> modifyer (+,-,*), Wavefunction 1, Wavefunction 2, etc 
    */
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )     
{
  if (nlhs == 1 && nrhs > 2) {
	if (mxIsChar(prhs[0]) && (!strcmp (mxArrayToString(prhs[0]),"+") || !strcmp (mxArrayToString(prhs[0]),"-") || !strcmp (mxArrayToString(prhs[0]),"*") || !strcmp (mxArrayToString(prhs[0]),"test") )) {
	    ...
	    } else if (!strcmp (mxArrayToString(prhs[0]),"*") ) {
	      for (int i = 2;i<nrhs;i++) {
		  if (i == 2) {
