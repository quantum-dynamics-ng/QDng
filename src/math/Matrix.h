 ...
 *
 * Data will not be initailized with zeroes per default.
 * 
 * Ordering: row major.
 * <p>
...
class Matrix 
{
   ...
   */
    Matrix(lint M, lint N, const T* v)
    {
        initialize(M,N);
        copy(v);
    }
   ...
   */
    Matrix(lint M, lint N, const char *s)
    {
        initialize(M,N);
    ...
    */
    Matrix<T>& newsize(lint M, lint N)
    {
    ...
    */
    ...
    lint num_rows() const { return m_; }
    lint num_cols() const { return n_; }




    inline T* operator[](lint i)
    {
#ifdef TNT_BOUNDS_CHECK
        assert(0<=i);
        assert(i < m_) ;
    ...
    inline const T* operator[](lint i) const
    {
#ifdef TNT_BOUNDS_CHECK
        assert(0<=i);
        assert(i < m_) ;
    ...
    inline reference operator()(lint i, lint j)
    { 
#ifdef TNT_BOUNDS_CHECK
        assert(1<=i);
        assert(i <= m_) ;
    ...
    inline const_reference operator() (lint i, lint j) const
    {
#ifdef TNT_BOUNDS_CHECK
        assert(1<=i);
        assert(i <= m_) ;
