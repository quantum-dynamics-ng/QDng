namespace QDLIB {
	...
	#ifdef HAVE_AVX
			for (i=0; i < size; i++){
			   m256dc _res = m256dc((A->begin(s))[i]) + m256dc((B->begin(s))[i]) * (C->begin(s))[i];; _res.Store((A->begin(s))[i]);
			}
	#elif HAVE_SSE2
			for (i=0; i < size; i++){
			   m128dc _res = m128dc((A->begin(s))[i]) + m128dc((B->begin(s))[i]) * (C->begin(s))[i];; _res.Store((A->begin(s))[i]);
			}
	#else
			for (i=0; i < size; i++){
