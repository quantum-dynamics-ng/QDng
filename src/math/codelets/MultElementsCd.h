namespace QDLIB {
	 ...
	 */
	inline void MultElements (cVec* A, double d)
	{
	  lint size = A->lsize();
	...
	#ifdef HAVE_AVX
			for (i=0; i < size; i++){
			   m256dc _res = m256dc((A->begin(s))[i]) * d; _res.Store((A->begin(s))[i]);
			}
	#elif HAVE_SSE2
			for (i=0; i < size; i++){
			   m128dc _res = m128dc((A->begin(s))[i]) * d; _res.Store((A->begin(s))[i]);
			}
	#else
			for (i=0; i < size; i++){
