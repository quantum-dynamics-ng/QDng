AC_DEFUN([ACX_FFTW3],[
  ...
  if test "x$with_fftw3" = x; then
    ...
    if test "x$enable_openmp" = "xyes"; then
    	AC_CHECK_LIB(fftw3_threads, fftw_init_threads,[FFTW_LIBS="-lfftw3 -lfftw3_threads"],AC_MSG_ERROR(fftw3_threads libs are missing),[-lfftw3])
    ...
    if test "x$enable_openmp" = "xyes"; then
    	AC_CHECK_LIB(fftw3_threads, fftw_init_threads,[FFTW_LIBS="-lfftw3 -lfftw3_threads -L$with_fftw3/lib"],AC_MSG_ERROR(fftw3_threads libs are missing),[-lfftw3])
    	AC_MSG_NOTICE([Using fftw3 with threads])
    fi
