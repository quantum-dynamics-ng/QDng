...
\newcommand{\ddx}[1]{{\partial_{#1}}}
\newcommand{\dddx}[1]{{\partial^2_{#1}}}
\newcommand{\dfdx}[2]{\dfrac{\partial#1}{\partial#2}}
...
\subsection{Jump}
Random jump operator for statistical quantum jump calculations in spirit of the
Lindblad equation.
The jump operator applies the provided operators at random times and is supposed
to be used in the \code{filterpost} section.

The jump propability is caclulated as
\begin{equation}
  P = \Delta t \langle \Psi \vert L^\dagger L \vert \Psi \rangle\,,
\end{equation}
and the provided Lindblad operators are applied, when $P$ is larger
than the drawn random number. For a full setup corresponding
non-Hermitian terms, in the form of $-iL^\dagger L$, in the Hamiltonian are also required.

Note: the operator also normalizes the wave function at each application.

\begin{options}
\option{seed}{Seed for the random number generator. Positive 48 bit integer [0].}
\option{max\_pjump}{Maximum accepted jump propability [0.5].}
\end{options}

\subsubsection*{Example}
\begin{verbatim}
qdng {
  propa { steps=1000 dt=0.1
    propagator { name=Arnoldi
      hamiltonian { name=Sum
        H0 { name=HermitianMatrix file=H0 }
        Hi { name=Product
          i { name=Scalar value=0 im=-1 }
          L1 { name=HermitianMatrix file=LdaggerL }
        }
      }
    }
    wf { file=psi }
    filterpost {
      apply { name=Jump seed=2 max_pjump=0.1
        L1 { name=HermitianMatrix file=L }
      }
    }
  }
}
